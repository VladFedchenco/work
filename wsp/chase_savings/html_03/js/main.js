let ctx, imgs_loaded, light_counter, fans_counter, win_prize_counter;
...
request.onload = function() {
  ...
  if(prizeData.prize_ID >= 2810 && prizeData.prize_ID <= 2817) {
    prefix = "";
  } else if (prizeData.prize_ID >= 2830 && prizeData.prize_ID <= 2837) {
    $("#main_header").setAttribute("class", "associates");
    prefix = "g_";
  }
...
function init() {
  ...
  wheel_base.src = 'imgs/' + prefix + 'wheel_base.png';
  img_preload[0] = false;
  ...
  wheel_sectors.src = 'imgs/' + prefix + 'wheel_sectors.png';
  img_preload[1] = false;
  ...
  wheel_ridge.src = 'imgs/' + prefix + 'wheel_ridge.png';
  img_preload[3] = false;
  ...
  wheel_center.src = 'imgs/' + prefix + 'wheel_center.png';
  img_preload[9] = false;
  ...
  win_prize_counter = 0;
  win_prize = new Image();
  win_prize.src = 'imgs/' + prefix + 'sector' + win_sector + '.png';
  ...
  fans_counter = 0;
  fans = new Image();
  fans.src = 'imgs/fans_bg.png';
  img_preload[11] = false;
  fans.onload = function() {
    for (let i = 0; i < 36; i++) {
      fans_frames[i] = document.createElement('canvas');
      let pctx = fans_frames[i].getContext('2d');
      fans_frames[i].width = 508;
      fans_frames[i].height = 508;
      let x = 254 - scale[i] * 508 / 2;
      let y = 508 - scale[i] * 508;
      pctx.drawImage(fans, x, y, scale[i] * 508, scale[i] * 508);
    }
    img_preload[11] = true;
  }

  wheel_blr = new Image();
  wheel_blr.src = 'imgs/' + prefix + 'blr' + win_sector + '.png';
  img_preload[12] = false;
  wheel_blr.onload = function() {
    img_preload[12] = true;
  }
    ...
    if(!game_start) {
      ...
      if(pause < 200) {
        ...
        if(rotate_counter == (angles[win_sector].length - 1)) {
          ctx.clearRect(0, 0, 750, 860);
          ctx.drawImage(wheel_blr, 0, 0, 750, 860);
          win_prize_counter < 35 ? win_prize_counter++ : win_prize_counter = 35;
          ctx.drawImage(fans_frames[win_prize_counter], 121, 180, 508, 508);
...
function win_info() {
  $("#ttl1").setAttribute("class", "invis");
  $("#ttl2").setAttribute("class", "");
  $("#prize_texts").setAttribute("class", "");
  if(game_version == "v0") {
    $("#remaining_spins").setAttribute("class", "");
  }
  if(game_version == "v1") {
    $("#claim_prize").setAttribute("class", "");
  }
  if(game_version == "v2") {
    $("#get_more_spins").setAttribute("class", "");
    $("#bttn_home").setAttribute("class", "bttn");
  }
  if(game_version == "v3") {
    $("#get_more_spins").setAttribute("class", "");
  }
